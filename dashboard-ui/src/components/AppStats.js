...
export default function AppStats() {
  ...
  const getStats = () => {
    fetch(`http://kafka-lab6.canadacentral.cloudapp.azure.com/processing/stats`)
      .then((res) => res.json())
      .then(
        (result) => {
      ...
      <div>
        ...
        <table className={"StatsTable"}>
          <tbody>
            ...
            <tr>
              <td colspan="2">
                Max Direction Reading: {stats["max_direction_readings"]}
              </td>
            </tr>
            <tr>
              <td colspan="2">
                Max Scale Reading: {stats["max_scale_readings"]}
              </td>
            </tr>
          </tbody>
        </table>
